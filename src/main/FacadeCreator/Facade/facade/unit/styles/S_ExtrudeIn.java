...
public class S_ExtrudeIn extends BasicObject {
    ...
    private ArrayList<HE_Mesh> glasses_updates;

    @Override
    protected void initPara() {
        top_height = putPara(120, 100, 1000, "top_height").getValue();
        bottom_height = putPara(100, 100, 1000, "bottom_height").getValue();
        top_depth = putPara(900, 100, 1500, "top_depth").getValue();
        glass_offset = putPara(300, 0, 1500, "glass_offset").getValue();
        borderWidth = putPara(50, 50, 300, "borderWidth").getValue();
        extrudeIn_up_height = putPara(900, 0, 900, "extrudeIn_up_height").getValue();
        divideNum = putPara(2, 1, 5, "divideNum").getValue();
        seed = putPara(0, 0, 20, "seed").getValue();
    }
...
//        StyledPolyLine styledPolyLine = new StyledPolyLine(0x00000000,4).add(frame);
        ...
        //*******************************************just for test
        //divide frames
        ArrayList<WB_PolyLine> dividedFrames = randomDivide(frame);
//        styledPolyLine.addAll(dividedFrames);

        //addLREdges
        ArrayList<HE_Mesh> LREdges = addLREdges(dividedFrames);
        whiteMesh.addAll(LREdges);
        glasses_updates = new ArrayList<>();

        //applyExtrudeIn();
//        applyExtrudeIn(dividedFrames);
